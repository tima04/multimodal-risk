...
def save_data(data, partial=False, is_fmri = False):
    ...
    if run:
        tag = "fmri" if is_fmri else "behavioral"
        file_ = "id-{0}_run-{1}_date-{2}_{3}_choice_data.json".\
                format(id_, run, date, tag)
    elif play:
...
def random_binary_generator2(p, k=3):
